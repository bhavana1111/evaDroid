...
start_activity = "com.expedia.bookings/.activity.SearchActivity"
package_name = "com.expedia.bookings"
...
def extract_clickables(xml_path):
    ...
    def traverse(node, parent=None):
        ...
        # Filter out meaningless containers unless they have a label
        if is_clickable:
    ...
    traverse(root)
    sig = hashlib.md5("".join([c["res_id"] + c["bounds"] for c in clickables]).encode()).hexdigest()
    return clickables, sig
...
def log_for_vectorization(clickables, history, button):
    ...
    with open("vector_log_local_random.txt", "a", encoding="utf-8") as f:
...
def is_actionable(c):
    ...
    # Ignore generic / decorative items
...
while time.time() - start_time < max_duration:
    ensure_in_test_app()
    create_dump()
    clickables, sig = extract_clickables(XML_PATH)

    screen_visit_counts[sig] += 1
    if screen_visit_counts[sig] == 3:
        scroll_down()
        screen_scroll_attempts[sig]+=1
        if screen_scroll_attempts[sig]>=2:
            restart_app()
        continue
    if screen_visit_counts[sig] > 5:
        restart_app()
        continue
    if not clickables:
    ...
    selected = random.choice(actionables)
    name = selected["text"] or selected["desc"] or get_display_name_from_node_and_children(selected["node"]) or selected["class"]

    print("🔘 Button:", name)

    if tap(selected["bounds"]):
        history.append(selected)
        log_for_vectorization(clickables, history, name)
        time.sleep(3)
    else:
        print("❌ Tap failed. Restarting.")
        restart_app()
    time.sleep(5)
